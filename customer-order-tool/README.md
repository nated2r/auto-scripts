...
本機網頁小工具：上傳 **AgentONE 顧客名單** 與 **訂單 CSV**，用姓名對到顧客，匯出時 `customerId` 寫入**平台 ID**（非 AgentONE 用戶 ID）。同名／對不到只顯示在畫面，不進下載檔。
...
1. **顧客名單**：上傳 AgentONE 匯出 CSV（需含 `顯示名稱`、`平台 ID`；建議含 `電話/手機`、`位置（現居城市）`）。匯出**不會**使用 `AgentONE 用戶 ID`。
...
| 情況 | 行為 |
|------|------|
| 姓名正規化後剛好 1 位顧客 | 寫入匯出：`customerId`←**平台 ID**；`mobile`←訂單電話（空則顧客電話）；`count` 空→`1`；`type` 空→`一般訂單` |
| 同名 ≥2 | 僅「同名待確認」表 |
| 0 筆 | 僅「對不到」表 |

### 顧客欄位別名（節錄）

- 顯示名稱：`顯示名稱`／`displayname`…
- 平台 ID（→ 匯出 `customerId`）：`平台 ID`／`平台ID`／`platform id`／`platform_id`（**不含** `AgentONE 用戶 ID`、泛用「用戶 id」）
