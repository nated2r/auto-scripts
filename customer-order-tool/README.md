...
- 姓名（優先順序）：`收件姓名`／`收件人`＞`顧客姓名`／`客戶姓名`／`姓名`＞`訂購人`（店家帳號名常為訂購人，故排最後）
- 電話：`收件電話`／`電話`／`手機`／`mobile`…
- 日期：`訂單日期`／`訂購日期`／`下單日期`／`date`…（`2026-09-22/202638` 會正規化為 `2026-09-22`）
- 數量：`數量`／`件數`／`qty`…
- 價格：`訂單金額`／`價格`／`金額`／`price`…
- 備註：`備註`／`note`…
...
會用 `fixtures/customers-sample.csv` + `fixtures/orders-sample.csv`（及別名／LuckyCat 訂單）驗證比對與匯出 BOM。
